...
const [sortOrder, setSortOrder] = useState<string>('desc');
  ...
  useEffect(() => {
    loadStrategies();
    // 加入依賴：當 sortBy 或 sortOrder 改變時，自動重新載入
  }, [sortBy, sortOrder]);
...
<Box sx={{ mb: 3, display: 'flex', gap: 2, alignItems: 'center', justifyContent: 'flex-end' }}>
        <FormControl size="small" sx={{ minWidth: 140, bgcolor: 'rgba(255,255,255,0.05)', borderRadius: 1 }}>
          <InputLabel sx={{ color: 'rgba(255,255,255,0.7)' }}>排序依據</InputLabel>
          <Select
            value={sortBy}
            label="排序依據"
          ...
          >
            ...
            <MenuItem value="strategy_type">策略類型</MenuItem>
            <MenuItem value="name">策略名稱</MenuItem>
            <MenuItem value="initial_capital">初始資金</MenuItem>
          </Select>
        </FormControl>

        <FormControl size="small" sx={{ minWidth: 140, bgcolor: 'rgba(255,255,255,0.05)', borderRadius: 1 }}>
          <InputLabel sx={{ color: 'rgba(255,255,255,0.7)' }}>順序</InputLabel>
          <Select
            value={sortOrder}
            label="順序"
            ...
            sx={{ color: 'white', '.MuiSvgIcon-root': { color: 'white' } }}
          >
            <MenuItem value="desc">由新到舊 (降冪)</MenuItem>
            <MenuItem value="asc">由舊到新 (升冪)</MenuItem>
          </Select>
        </FormControl>
      </Box>
