...
strategy = {
    ...
    "grid_lower_price": 0,  # Auto-set
    "grid_upper_price": 0,  # Auto-set
    "grid_num_grids": 10,
...
try:
    response = requests.post(url, json=strategy, timeout=60)
    if response.status_code == 200:
        ...
        print(f"Strategy: Grid Trading ({strategy['grid_num_grids']} grids)")
        print("="*70)
        ...
        print(f"\n{'='*70}")
        print("Sample Trade History (first 10 trades):")
        print("="*70)
        ...
        print(f"[FAIL] Backtest failed: {response.status_code}")
        print(response.text)
